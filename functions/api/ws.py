...
class api(commands.Cog):
    ...
    def weather_embed(self, stats_json_array, unit):
        ...
        try:
            ...
            ws_weather0 = ws['weather'][0]
            ws_sys = ws['sys']
            ws_main = ws['main']
            # creation of embed with all important OpenWeatherMap's json info
            em = {
                "title": f"Weather in **{ws['name']}**, **{ws_sys['country']}**",
                "description": f"{emoji} {ws_weather0['main']}, {ws_weather0['description']}",
                "color": color,
                "thumbnail": {"url": f"https://openweathermap.org/img/wn/{ws_weather0['icon']}@2x.png"},
                "fields": [
                    {
                        "name": "Weather",
                        "value":
                            f"**Temperature**: {ws_main['temp']}{unit['temp']}, feels like {ws_main['feels_like']}{unit['temp']}\n"
                            f"**Wind**: {ws['wind']['speed']}{unit['speed']}\n"
                            f"**Clouds**: {ws['clouds']['all']}%\n"
                            f"**Humidity**: {ws_main['humidity']}%\n",
                        "inline": True
                    },{
                        "name": "Time",
                        "value":
                            f"**Sunrise**: {datetime.fromtimestamp(ws_sys['sunrise']+ws['timezone']).strftime('%H:%M')}\n"
                            f"**Sunset**: {datetime.fromtimestamp(ws_sys['sunset']+ws['timezone']).strftime('%H:%M')}\n"
                    }
                ],
